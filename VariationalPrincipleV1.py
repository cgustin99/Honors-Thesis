...
psi = np.zeros(N)
psi = np.fromiter((N - i for i in range(len(psi))), float)
...
for iterations in range(100000):
    ...
    h_shift = random.uniform(-0.5 * npsi[k], 0.5 * npsi[k])
...
#print("accept1:", accept_1, "accept2:", accept_2)'''
    print(iterations, expH)
    arr.append(expH - (-13.6))
 
 
plt.plot(np.abs(arr), color = 'blue')
plt.yscale('log')
plt.ylabel('Numeric Energy - Analytic Energy')
